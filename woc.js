...
fetch('https://api.themoviedb.org/3/movie/popular?language=en-US&page=1', fetchPopular)
 .then(response => response.json())
 .then(data =>{
    console.log(data)
...
fetch('https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=1', fetchUpcoming)
 .then(response => response.json())
 .then(data =>{console.log(data)
    ...
    const movies = data.results;
    const movieGrid = document.querySelector('.movieGrid3')

    movies.forEach(movie => {
        const{title, overview , poster_path, genre_ids}= movie;
        const gamma = document.createElement('div');
        gamma.classList.add('movieCard3');
        gamma.innerHTML=` 
       ...
       <div>
        ...
        const movieTitle = document.createElement('div');
        movieTitle.classList.add('movieTitle3');
        movieTitle.innerText = gamma.title;

        movieGrid.appendChild(gamma);
...
mov3[0].style.display='none'
 ...
 const fetchTOP = {
  ...
  const horror = {
    ...
    const apiUrl2 = 'https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=1';
